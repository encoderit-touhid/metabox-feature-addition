...
function property_gallery_add_metabox(){
	add_meta_box(
		...
		POST_TYPE, // Change post type name
		'normal',
		'core'
	);
}
add_action( 'admin_init', 'property_gallery_add_metabox',15);
	...
	<div id="gallery_wrapper">
			...
			<div class="gallery_single_row dolu">
			  <div class="gallery_area image_container ">
				<?php 
				 ...
				 {
					?><img class="gallery_img_img" src="https://cdn-icons-png.flaticon.com/512/179/179483.png" height="55" width="55" onclick="slider_start()"/><?php

				 }else
				 {
                   ?><img class="gallery_img_img" src="<?php esc_html_e( $gallery_data['image_url'][$i] ); ?>" height="55" width="55" onclick="slider_start()"/><?php
				 }
				?>
		...
		<div style="display:none" id="master_box">
			<div class="gallery_single_row">
				<div class="gallery_area image_container" onclick="slider_start()">
...
function property_gallery_styles_scripts(){
    ...
    <script type="text/javascript">
        function remove_img(value) {
            ...
            parent.remove();
            //jQuery('.custom_slider').empty()
			console.log(jQuery(image_urls));
           

			//jQuery('.slider__item').last().remove();
        }
		...
		function open_media_uploader_image_plus(){
			...
			media_uploader.on("insert", function(){
				...
				for(var i = 0; i < length; i++){
					...
					let file_type=image_url.split('.').pop().toLowerCase();
					...
					if(file_type == "pdf")
					{
						...
						var html = '<img class="gallery_img_img" src="https://cdn-icons-png.flaticon.com/512/179/179483.png" height="55" width="55" onclick="slider_start()"/>';

						var slider_pdf=`<iframe class="slider__item" src="${image_url}"></iframe>`
						jQuery('.custom_slider').append(slider_pdf);
						slider_functionalities();
                        //image_save_on_import();
						image_urls.push(`${image_url}`)
						
					}else
					{
						
						var html = '<img class="gallery_img_img" src="'+image_url+'" height="55" width="55" onclick="slider_start()"/>';
						var slider_img=`<img class="slider__item" src="${image_url}"alt="Slider Image"/>`
						jQuery('.custom_slider').append(slider_img);
						slider_functionalities()
						image_urls.push(`${image_url}`)
					}
					element.append(html);
...
add_action( 'admin_head-post.php', 'property_gallery_styles_scripts',15 );
add_action( 'admin_head-post-new.php', 'property_gallery_styles_scripts',15 );
...
function property_gallery_save( $post_id ) {
	...
	// Correct post type
	if ( POST_TYPE != $_POST['post_type'] ) // here you can set the post type name
		return;
